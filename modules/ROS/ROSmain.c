...
void ROS_MAIN_LOOP(void) {
    ...
    while (1) {
        ...
        switch (result) {
            ...
            case 2:
                // INSfunc의 복사 로직 재활용
                // [수정] INS_copy_to_sector 시그니처가
                // (WINDOW*, src_path, dest_path, sector_name) 4개 인자로 바뀌었는데
                // 여기서는 옛날 3-인자 방식으로 호출하고 있었음.
                // -> sector_name 자리에 아무 값도 안 들어가 스택 쓰레기값이 들어가고,
                //    그 쓰레기 포인터로 snprintf(title, ..., sector_name)를 호출하면서
                //    메뉴 선택 즉시 크래시(화면 깨짐)가 발생했던 것.
                // ROS -> EDS 복사이므로 소스 경로는 ROS_PATH로 명시.
                INS_copy_to_sector(right_data_win, ROS_PATH, EDS_PATH, "EDS");
                break;
            case 3:
                goto cleanup;
        }
